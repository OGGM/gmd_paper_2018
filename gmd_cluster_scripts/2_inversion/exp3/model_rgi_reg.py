...
execute_entity_task(tasks.prepare_for_inversion, gdirs, invert_with_rectangular=False)

for f in factors:
    suf = '_parab_{:03d}_nofs'.format(int(f * 10))
...
for f in factors:
    suf = '_parab_{:03d}_wfs'.format(int(f * 10))
